...
from src.second_opinion.core.models import ModelRequest, Message
...
class TestCostGuard:
    ...
    async def test_get_detailed_analytics(self):
        ...
        await self._add_mock_usage(Decimal('2.00'), hours_ago=25, model="gpt-4", operation="second_opinion")
    ...
    async def test_reservation_cleanup(self):
        ...
        reservation.timestamp = datetime.now(timezone.utc) - timedelta(seconds=600)  # 10 minutes ago
...
class TestEdgeCases:
    ...
    async def test_budget_period_edge_cases(self):
        ...
        with patch('src.second_opinion.utils.cost_tracking.datetime') as mock_dt:
            mock_dt.now.return_value = datetime(2023, 12, 31, 23, 59, 59, tzinfo=timezone.utc)
