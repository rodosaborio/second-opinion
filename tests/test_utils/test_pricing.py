...
class TestPricingManager:
    ...
    @pytest.fixture
    def pricing_manager(self, temp_dir, sample_litellm_data):
        """Create pricing manager with test data."""
        cache_file = temp_dir / "pricing_cache.json"
        backup_file = temp_dir / "pricing_backup.json"
    ...
    def test_cache_save_and_load(self, pricing_manager, temp_dir):
        ...
        new_manager = PricingManager(
            ...
            backup_file=temp_dir / "empty_backup.json",
            cache_ttl_hours=1
        )
    ...
    def test_thread_safety(self, pricing_manager):
        ...
        assert all(r == Decimal("0.012") for r in results)
    ...
    def test_parse_litellm_data_invalid_entries(self, pricing_manager):
        ...
        assert "invalid-model-1" not in parsed_data
        assert "invalid-model-2" not in parsed_data
...
class TestPricingManagerEdgeCases:
    ...
    def test_empty_backup_file(self, temp_dir):
        """Test handling of empty backup file."""
        empty_backup = temp_dir / "empty.json"
    ...
    def test_corrupted_backup_file(self, temp_dir):
        """Test handling of corrupted backup file."""
        corrupted_backup = temp_dir / "corrupted.json"
    ...
    def test_missing_backup_file(self, temp_dir):
        """Test handling of missing backup file."""
        manager = PricingManager(
            cache_file=temp_dir / "cache.json",
            backup_file=temp_dir / "nonexistent.json"
        )
    ...
    def test_cache_file_permissions(self, temp_dir):
        ...
        cache_file = temp_dir / "readonly_cache.json"
    ...
    def test_extremely_large_dataset(self, temp_dir):
        ...
        backup_file = temp_dir / "large_backup.json"
        with open(backup_file, 'w') as f:
