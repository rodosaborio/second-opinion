...
def mock_consultation_dependencies(monkeypatch):
    ...
    mock_cost_guard.check_and_reserve_budget = AsyncMock(return_value=MagicMock(
    ...
    monkeypatch.setattr("second_opinion.mcp.tools.consult.get_cost_guard", get_mock_cost_guard)
    
    # Mock evaluator
    mock_evaluator = MagicMock()
    mock_evaluator.classify_task_complexity = AsyncMock(return_value=TaskComplexity.MODERATE)
    
    def get_mock_evaluator():
        return mock_evaluator
    
    monkeypatch.setattr("second_opinion.mcp.tools.consult.get_evaluator", get_mock_evaluator)
    
    # Mock client factory
    mock_client = MagicMock()
    mock_client.complete = AsyncMock(return_value=ModelResponse(
    ...
    monkeypatch.setattr("second_opinion.mcp.tools.consult.create_client_from_config", create_mock_client)
    ...
    monkeypatch.setattr("second_opinion.mcp.tools.consult.detect_model_provider", mock_detect_provider)
    ...
    async def mock_classify_domain(query, context=None):
        ...
        if any(term in query_lower for term in ["python", "function", "code", "debug", "programming"]):
            return "coding"
        elif any(term in query_lower for term in ["performance", "optimize", "scalable", "infrastructure"]):
            return "performance"
        elif any(term in query_lower for term in ["creative", "story", "marketing", "content"]):
            return "creative"
        else:
            return "general"
    
    monkeypatch.setattr("second_opinion.mcp.tools.consult.classify_consultation_domain", mock_classify_domain)
...
class TestConsultationSession:
    ...
    async def test_add_turn(self):
        ...
        await session.add_turn("What is async programming?", "Async programming allows...", Decimal("0.05"))
...
class TestConsultationModelRouter:
    ...
    def test_model_recommendation_delegate(self):
        ...
        model = router.recommend_model("delegate", task_complexity=TaskComplexity.SIMPLE)
        assert model == "openai/gpt-4o-mini"
        
        # Complex task should use better model
        model = router.recommend_model("delegate", task_complexity=TaskComplexity.COMPLEX)
        assert model == "anthropic/claude-3-5-sonnet"
    ...
    def test_user_specified_model_override(self):
        ...
        model = router.recommend_model(
            "delegate", 
            user_specified_model="anthropic/claude-3-opus"
        )
        assert model == "anthropic/claude-3-opus"
...
class TestTurnController:
    ...
    async def test_single_turn_consultation(self, turn_controller, mock_consultation_dependencies):
        ...
        assert results["conversation_turns"][0]["response"] == "Mock consultation response"
        assert session.turn_count == 1
        assert session.status == "completed"
    
    async def test_multi_turn_consultation(self, turn_controller, mock_consultation_dependencies):
    ...
    async def test_cost_limit_stops_conversation(self, turn_controller, mock_consultation_dependencies):
