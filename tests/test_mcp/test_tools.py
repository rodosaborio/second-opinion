...
class TestSecondOpinionTool:
    ...
    async def test_basic_tool_execution(self):
        ...
        with patch("src.second_opinion.mcp.tools.second_opinion.get_evaluator") as mock_evaluator, \
            ...
            mock_guard.check_and_reserve_budget = AsyncMock(return_value=MagicMock(reservation_id="test-123"))
            mock_guard.record_actual_cost = AsyncMock()
            mock_guard.get_usage_summary = AsyncMock(return_value=MagicMock(available=Decimal("1.50")))
    ...
    async def test_input_validation(self):
        ...
        with patch("src.second_opinion.mcp.tools.second_opinion.sanitize_prompt") as mock_sanitize, \
             patch("src.second_opinion.mcp.tools.second_opinion.validate_model_name") as mock_validate_model:
    ...
    async def test_cost_limit_validation(self):
        ...
        with patch("src.second_opinion.mcp.tools.second_opinion.get_cost_guard") as mock_cost_guard:
            
            # Mock cost guard that rejects due to budget
            mock_guard = MagicMock()
            mock_guard.check_and_reserve_budget = AsyncMock(side_effect=Exception("Budget exceeded"))
            mock_cost_guard.return_value = mock_guard
    ...
    async def test_auto_model_selection(self):
        ...
        with patch("src.second_opinion.mcp.tools.second_opinion.get_evaluator") as mock_evaluator, \
             patch("src.second_opinion.mcp.tools.second_opinion.get_cost_guard") as mock_cost_guard, \
             patch("src.second_opinion.mcp.tools.second_opinion.create_client_from_config") as mock_client_factory, \
             patch("src.second_opinion.cli.main.ComparisonModelSelector") as mock_selector_class:
            
            # Setup mocks
            mock_eval = MagicMock()
            mock_eval.classify_task_complexity = AsyncMock(return_value=TaskComplexity.MODERATE)
            mock_evaluator.return_value = mock_eval
    ...
    async def test_error_handling(self):
        ...
        with patch("src.second_opinion.mcp.tools.second_opinion.create_client_from_config") as mock_client_factory:
    ...
    async def test_think_tag_filtering(self):
        ...
        with patch("src.second_opinion.mcp.tools.second_opinion.get_evaluator") as mock_evaluator, \
             patch("src.second_opinion.mcp.tools.second_opinion.get_cost_guard") as mock_cost_guard, \
             patch("src.second_opinion.mcp.tools.second_opinion.create_client_from_config") as mock_client_factory, \
             patch("src.second_opinion.mcp.tools.second_opinion.filter_think_tags") as mock_filter:
