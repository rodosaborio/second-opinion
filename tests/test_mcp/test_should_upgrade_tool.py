...
class TestShouldUpgradeTool:
    ...
    def mock_evaluator(self):
        ...
        evaluator = MagicMock()
        evaluator.classify_task_complexity = AsyncMock(return_value=TaskComplexity.MODERATE)
    ...
    async def test_basic_upgrade_functionality(
        self, sample_response, sample_task, mock_evaluator, mock_cost_guard, mock_client, monkeypatch
    ):
        """Test basic upgrade functionality with auto-selection."""
        # Mock dependencies
        monkeypatch.setattr("second_opinion.mcp.tools.should_upgrade.get_evaluator", lambda: mock_evaluator)
        monkeypatch.setattr("second_opinion.mcp.tools.should_upgrade.get_cost_guard", lambda: mock_cost_guard)
        monkeypatch.setattr("second_opinion.mcp.tools.should_upgrade.create_client_from_config", lambda x: mock_client)
        monkeypatch.setattr("second_opinion.mcp.tools.should_upgrade.detect_model_provider", lambda x: "openrouter")

        # Setup mocks
        mock_cost_guard.check_and_reserve_budget.return_value = MagicMock(reservation_id="test_reservation")
        ...
        mock_evaluation_result.reasoning = "Upgrade provides better algorithm efficiency"
    ...
    async def test_premium_model_inclusion(
        ...
        mock_evaluation_result.reasoning = "Premium model provides superior implementation"
        mock_evaluator.compare_responses.return_value = mock_evaluation_result
    ...
    async def test_local_model_upgrade(
        ...
        def mock_provider_detection(model):
            if "mlx" in model:
                return "lmstudio"
            return "openrouter"
        monkeypatch.setattr("second_opinion.mcp.tools.should_upgrade.detect_model_provider", mock_provider_detection)
        ...
        mock_evaluation_result.reasoning = "Cloud model provides better error handling and documentation"
        mock_evaluator.compare_responses.return_value = mock_evaluation_result
    ...
    async def test_budget_model_upgrade(
        ...
        mock_evaluation_result.reasoning = "Premium model provides exceptional optimization and documentation"
        mock_evaluator.compare_responses.return_value = mock_evaluation_result
...
"""
    ...
    async def test_cost_limit_enforcement(
        ...
        mock_cost_guard.check_and_reserve_budget.side_effect = Exception("Cost limit exceeded: $0.20 > $0.10")
    ...
    async def test_complex_task_handling(
        ...
        mock_evaluator.classify_task_complexity = AsyncMock(return_value=TaskComplexity.COMPLEX)
        mock_cost_guard.check_and_reserve_budget.return_value = MagicMock(reservation_id="test_reservation")
        ...
        mock_evaluation_result.reasoning = "Premium model excels at complex optimization tasks"
        mock_evaluator.compare_responses.return_value = mock_evaluation_result
    ...
    async def test_quality_improvement_calculations(
        ...
        assert "significant improvement" in result.lower() or "excellent" in result.lower()
        assert "Cost vs Quality Analysis" in result
    ...
    async def test_invalid_upgrade_candidates(self, sample_response, sample_task):
        ...
        result = await should_upgrade_tool(
            ...
            task=sample_task,
            current_model="anthropic/claude-3-haiku",
            upgrade_candidates=["@invalid-model!", "*another-invalid*"],  # Use clearly invalid formats with special chars
            include_premium=True
        )
