...
def check_env_file():
    ...
    if "OPENROUTER_API_KEY=your_openrouter_api_key_here" in content:
        ...
        print("   Edit .env and replace 'your_openrouter_api_key_here' with your actual key")
        return False
...
def check_configuration():
    ...
    try:
        ...
        print(f"   Cost limits: ${settings.cost_management.default_per_request_limit} per request")
...
def test_api_connection():
    ...
    try:
        ...
        result = subprocess.run(
            ["uv", "run", "second-opinion", "--primary-model", "anthropic/claude-3-5-sonnet", "Hello"],
            capture_output=True,
            text=True,
            timeout=30
        )
...
def main():
    ...
    if all_passed:
        ...
        print('   uv run second-opinion --primary-model "anthropic/claude-3-5-sonnet" "What\'s 2+2?"')
