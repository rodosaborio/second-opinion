...
class DatabaseConfig(BaseModel):
    ...
    path: str = Field(default="data/second_opinion.db", description="Database file path")
    encryption_enabled: bool = Field(default=True, description="Enable database encryption")
    backup_enabled: bool = Field(default=True, description="Enable automatic backups")
    backup_interval: int = Field(default=86400, ge=1, description="Backup interval in seconds")
    retention_days: int = Field(default=30, ge=1, description="Data retention period in days")


class SecurityConfig(BaseModel):
    """Security configuration."""
    input_sanitization: bool = Field(default=True, description="Enable input sanitization")
    response_filtering: bool = Field(default=True, description="Enable response filtering")
...
class CostManagementConfig(BaseModel):
    ...
    default_per_request_limit: Decimal = Field(default=Decimal("0.05"), ge=0, description="Default per-request cost limit")
    daily_limit: Decimal = Field(default=Decimal("2.00"), ge=0, description="Daily spending limit")
...
class APIConfig(BaseModel):
    ...
    max_backoff: int = Field(default=60, ge=1, description="Maximum backoff time in seconds")
...
class PerformanceConfig(BaseModel):
    ...
    cache_duration: int = Field(default=300, ge=0, description="Cache duration in seconds")
    max_response_size: int = Field(default=1048576, ge=1024, description="Maximum response size in bytes")
    connection_pool_size: int = Field(default=10, ge=1, le=100, description="HTTP connection pool size")
...
class AnalyticsConfig(BaseModel):
    ...
    retention_days: int = Field(default=90, ge=1, description="Analytics data retention in days")
...
class DevelopmentConfig(BaseModel):
    ...
    test_data_enabled: bool = Field(default=False, description="Enable test data generation")
