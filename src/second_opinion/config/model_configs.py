...
from ..core.models import ModelTier, ModelInfo, PricingInfo


class ToolConfig(BaseModel):
    """Configuration for a specific tool."""
    description: str = Field(..., description="Tool description")
    comparison_models: Optional[List[str]] = Field(None, description="Models to use for comparison")
    upgrade_targets: Optional[Dict[str, str]] = Field(None, description="Upgrade target models")
    downgrade_targets: Optional[List[str]] = Field(None, description="Downgrade target models")
    evaluator_model: Optional[str] = Field(None, description="Model for evaluation tasks")
    
    max_tokens: int = Field(default=500, ge=1, le=32000, description="Maximum tokens per request")
    temperature: float = Field(default=0.1, ge=0.0, le=2.0, description="Sampling temperature")
    cost_limit_per_request: Decimal = Field(default=Decimal("0.05"), ge=0, description="Cost limit per request")
    system_prompt_template: Optional[str] = Field(None, description="System prompt template name")
...
class ModelCapabilities(BaseModel):
    ...
    max_context: Optional[int] = Field(None, ge=1, description="Maximum context window")
    max_output: Optional[int] = Field(None, ge=1, description="Maximum output tokens")
    supports_system_messages: bool = Field(default=True, description="Supports system messages")
    supports_temperature: bool = Field(default=True, description="Supports temperature control")
    supports_streaming: bool = Field(default=True, description="Supports streaming responses")
    cost_per_input_token: Optional[Decimal] = Field(None, ge=0, description="Cost per input token")
    cost_per_output_token: Optional[Decimal] = Field(None, ge=0, description="Cost per output token")


class ModelTierConfig(BaseModel):
    """Configuration for model tiers."""
    budget: List[str] = Field(default_factory=list, description="Budget tier models")
    mid_range: List[str] = Field(default_factory=list, description="Mid-range tier models")
    premium: List[str] = Field(default_factory=list, description="Premium tier models")
    reasoning: List[str] = Field(default_factory=list, description="Reasoning-specialized models")
    ...
    def get_tier_for_model(self, model: str) -> Optional[ModelTier]:
...
class CostEstimates(BaseModel):
    ...
    input_token_costs: Dict[str, Decimal] = Field(default_factory=dict, description="Input token costs per 1K tokens")
    output_token_costs: Dict[str, Decimal] = Field(default_factory=dict, description="Output token costs per 1K tokens")
    
    def get_input_cost(self, model: str) -> Decimal:
        """Get input cost per 1K tokens for model."""
        return self.input_token_costs.get(model, Decimal("0.001"))  # Conservative default
    
    def get_output_cost(self, model: str) -> Decimal:
        """Get output cost per 1K tokens for model."""
        return self.output_token_costs.get(model, Decimal("0.004"))  # Conservative default
...
class DefaultSettings(BaseModel):
    ...
    cost_limit_per_request: Decimal = Field(default=Decimal("0.05"), ge=0, description="Default cost limit")
...
class ModelProfilesConfig(BaseModel):
    ...
    model_tiers: ModelTierConfig = Field(default_factory=ModelTierConfig, description="Model tier definitions")
    cost_estimates: CostEstimates = Field(default_factory=CostEstimates, description="Cost estimation data")
    defaults: DefaultSettings = Field(default_factory=DefaultSettings, description="Default settings")
    ...
    def get_comparison_models(self, tool_name: str, primary_model: str) -> List[str]:
        ...
        return [model for model in tool_config.comparison_models if model != primary_model]
    
    def get_upgrade_target(self, current_model: str, upgrade_type: str = "any_to_reasoning") -> Optional[str]:
        """Get upgrade target for a model."""
        for tool_config in self.tools.values():
            if tool_config.upgrade_targets and upgrade_type in tool_config.upgrade_targets:
...
class ModelConfigManager:
    ...
    def __init__(self):
        self._config: Optional[ModelProfilesConfig] = None
    ...
    def get_model_config(self, tool_name: str, primary_model: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if tool_config.comparison_models:
            config['comparison_models'] = self._config.get_comparison_models(tool_name, primary_model or "")
    ...
    def estimate_cost(self, model: str, input_tokens: int, output_tokens: int) -> Decimal:
        ...
        cost = self._config.cost_estimates.estimate_cost(model, input_tokens, output_tokens)
        
        # If no specific pricing data exists, return conservative fallback
        if (model not in self._config.cost_estimates.input_token_costs and 
            model not in self._config.cost_estimates.output_token_costs):
            return Decimal("0.10")
    ...
    def get_model_capability(self, model: str) -> Optional[ModelCapabilities]:
    ...
    def validate_model_request(self, model: str, max_tokens: int, has_system_prompt: bool) -> List[str]:
        ...
        if capability.max_output and max_tokens > capability.max_output:
            warnings.append(f"Requested {max_tokens} tokens exceeds model limit of {capability.max_output}")
...
def get_model_config(tool_name: str, primary_model: Optional[str] = None) -> Dict[str, Any]:
