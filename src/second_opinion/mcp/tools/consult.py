...
class ConsultationSession(MCPSession):
    ...
    def __init__(self, consultation_type: str, target_model: str, session_id: str | None = None):
        """
        ...
        logger.debug(f"Created consultation session {self.session_id} for {consultation_type} with {target_model}")
    ...
    async def add_turn(self, query: str, response: str, cost: Decimal) -> None:
        ...
        logger.debug(f"Session {self.session_id}: Added turn {self.turn_count}, cost ${cost:.4f}")
    ...
    async def conduct_consultation(
        ...
        for turn in range(max_turns):
            if not session.can_continue(max_turns, cost_limit):
                logger.info(f"Session {session.session_id}: Cannot continue - turn {turn}, cost ${session.total_cost:.4f}")
                break
            ...
            try:
                ...
                logger.error(f"Turn {turn + 1} failed for session {session.session_id}: {e}")
        ...
        try:
            ...
            logger.warning(f"Failed to generate summary for session {session.session_id}: {e}")
            results["consultation_summary"] = "Summary unavailable due to processing error."
...
Please provide a thoughtful response that builds on our previous discussion."""
    ...
    def _get_system_prompt(self, session: ConsultationSession) -> str:
        ...
        if session.consultation_type == "quick":
            return f"{base_prompt} Provide a concise, expert opinion with clear reasoning."
        elif session.consultation_type == "delegate":
            return f"{base_prompt} Complete the requested task efficiently and accurately."
    ...
    async def _assess_follow_up_need(
        ...
        if needs_follow_up and turn < max_turns - 1 and session.consultation_type in ["deep", "brainstorm"]:
    ...
    async def _extract_recommendations(self, session: ConsultationSession) -> List[str]:
        ...
        if session.consultation_type == "delegate":
            recommendations.append("Review the completed task for accuracy and completeness")
            ...
            recommendations.append("Review the comprehensive analysis and choose your approach")
            ...
            recommendations.append("Evaluate the explored alternatives based on your constraints")
...
async def consult_tool(
    ...
    try:
        ...
        logger.info(f"Starting consult tool: type={consultation_type}, query_length={len(clean_query)}, target_model={target_model}")
        ...
        if session_id:
            ...
            domain = await model_router.detect_domain_specialization(clean_query, clean_context)
            
            # Create new session
            session = create_consultation_session(consultation_type, recommended_model)
            session.domain = domain
            
            logger.info(f"Created session {session.session_id} with {recommended_model} for {domain} domain")
        ...
        try:
            budget_check = await cost_guard.check_and_reserve_budget(
                estimated_cost, "consult", session.target_model, 
...
def _format_consultation_response(
    ...
    if len(results["conversation_turns"]) > 1:
        ...
        for turn_data in results["conversation_turns"]:
            ...
            sections.append(f"**Query**: {turn_data['query'][:200]}{'...' if len(turn_data['query']) > 200 else ''}")
            sections.append("")
            sections.append(f"**Response**: {turn_data['response'][:800]}{'...' if len(turn_data['response']) > 800 else ''}")
            sections.append("")
    ...
    if consultation_type == "delegate":
        ...
        sections.append(f"**Cost Efficiency**: {((estimated_savings / (estimated_savings + results['total_cost'])) * 100):.1f}% savings")
    ...
    if consultation_type in ["deep", "brainstorm"] and session.status == "completed":
        ...
        sections.append(f"**Session ID**: `{session.session_id}`")
        sections.append("Use this session ID to continue the conversation with follow-up questions.")
        sections.append("")
    ...
    if consultation_type == "delegate":
        ...
        sections.append("3. **Consider similar delegations** for routine tasks to save time and cost")
        ...
        sections.append("3. **Use deep consultation** for more complex follow-up questions")
    elif consultation_type == "deep":
        sections.append("1. **Review the comprehensive analysis** and select your preferred approach")
        sections.append("2. **Start implementation** with the highest-priority recommendations")
        sections.append("3. **Continue the session** with specific implementation questions if needed")
    elif consultation_type == "brainstorm":
        sections.append("1. **Evaluate the explored alternatives** based on your specific constraints")
        sections.append("2. **Prototype the most promising approaches** to validate assumptions")
        sections.append("3. **Combine insights** from multiple solutions for an optimal approach")
