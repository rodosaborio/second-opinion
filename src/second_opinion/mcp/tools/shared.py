...
def validate_model_candidates(
    ...
    for i, model in enumerate(candidates):
        try:
            ...
            raise Exception(f"Invalid model candidate #{i+1} ({model}): {str(e)}{suggestion_text}") from e
...
def get_model_tier(model: str) -> str:
    ...
    # Budget tier
    if any(budget in model_lower for budget in [
        "haiku", "mini", "flash", "gemini-flash", "gpt-3.5"
    ]):
        return "budget"

    # Premium tier
    if any(premium in model_lower for premium in [
        "opus", "claude-3-opus", "gpt-4o", "gpt-4", "gemini-pro-1.5"
    ]):
        return "premium"

    # Mid-tier (default for sonnet and similar)
    if any(mid in model_lower for mid in [
        "sonnet", "claude-3-5-sonnet", "gemini-pro"
    ]):
        return "mid-tier"
...
def get_cross_provider_alternatives(current_model: str, target_tier: str) -> list[str]:
    ...
    if target_tier == "budget":
        ...
        for option in budget_options:
            ...
            if option_provider != current_provider and option.lower() not in current_lower:
...
def format_cost_comparison(
    ...
    if is_upgrade and cost_diff > 0:
        return cost_diff, f"+{percentage:.0f}%", f"+${cost_diff:.4f} ({percentage:.0f}% increase)"
    elif not is_upgrade and cost_diff < 0:
        return cost_diff, f"-{percentage:.0f}%", f"Save ${abs(cost_diff):.4f} ({percentage:.0f}% savings)"
    else:
        return cost_diff, f"{percentage:.0f}%", f"${abs(cost_diff):.4f} ({'increase' if cost_diff > 0 else 'savings'})"
...
def should_recommend_change(
    ...
    if is_upgrade:
        ...
        if score < quality_threshold:
            return False, f"Quality improvement insufficient ({score:.1f}/10 < {quality_threshold})"
        ...
        if current_cost > 0:
            ...
            if cost_ratio > cost_ratio_threshold:
                return False, f"Cost increase too high ({cost_ratio:.1f}x > {cost_ratio_threshold}x)"
